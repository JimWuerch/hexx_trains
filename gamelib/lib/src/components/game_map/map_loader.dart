...
class MapLoader {
  ...
  static MapData load(String map) {
    late MapOrientation orientation;
    late bool aRowOdd;
    late bool lettersVertical;
    late List<MapTile> mapTiles;
            ...
            if (node.nodeType == xml.XmlNodeType.ELEMENT) {
              ...
              if (element.name.local == 'tiles') {
                mapTiles = _parseTiles(element, aRowOdd, lettersVertical,
                    orientation == MapOrientation.pointy);
                ...
                offmapRevenue = _parseOffmapRevenue(element, aRowOdd,
                    lettersVertical, orientation == MapOrientation.pointy);
              } else {
                throw ArgumentError(
  ...
  static List<MapTile> _parseTiles(xml.XmlElement tilesElement, bool aRowOdd,
      bool lettersVertical, bool isPointy) {
  ...
  static List<Barrier> _parseBarriers(xml.XmlElement barriersElement,
      bool aRowOdd, bool lettersVertical, bool isPointy) {
  ...
  static List<MapText> _parseMapText(xml.XmlElement mapTextElement,
      bool aRowOdd, bool lettersVertical, bool isPointy) {
    var mapText = <MapText>[];
  ...
  static List<Terrain> _parseTerrains(xml.XmlElement terrainsElement,
      bool aRowOdd, bool lettersVertical, bool isPointy) {
  ...
  static List<Doodad> _parseDoodads(xml.XmlElement doodadsElement, bool aRowOdd,
      bool lettersVertical, bool isPointy) {
    var doodads = <Doodad>[];
    for (var node in doodadsElement.children) {
      if (node.nodeType == xml.XmlNodeType.ELEMENT) {
        var element = node as xml.XmlElement;
        late math.Point<int> location;
        ...
        doodads.add(Doodad(
            ...
            location: math.Point<int>(location.x, location.y)));
      }
    }
    return doodads;
  }

  static List<OffmapRevenue> _parseOffmapRevenue(xml.XmlElement revenueElement,
      bool aRowOdd, bool lettersVertical, bool isPointy) {
