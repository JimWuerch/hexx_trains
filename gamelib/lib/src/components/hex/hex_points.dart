...
class HexPoints {
  ...
  factory HexPoints.fromLayout(HexLayout layout) {
    ...
    var xTightCurveCP =
        List.generate(6, (index) => layout.hexSideOffset(index, .5) * tightCPScale, growable: false); //new SKPoint[6];;

    // must calc CurvePoints first, as this uses them
    var xCurvePoints = List.generate(6, (index) => calcCurvePoints(index, layout, xTightCurveCP, xGentleCurveCP),
    ...
    var xOutsideCorners = List.generate(7, (index) => index == 6 ? xCorners[4][0] : xCorners[4][index],
    ...
    var xLevel1SideCities =
        List.generate(6, (index) => layout.hexSideOffset(index, .25 + .25 / 2.0), growable: false); //new SKPoint[6];

    var xLevel1CornerCities =
        List.generate(6, (index) => layout.hexCornerOffset(index, .25 + .25 / 2.0), growable: false); // new SKPoint[6];
  ...
  static CurvePoint calcCurvePoints(
    ...
    // Curve 1 R
    var cdr1 = CurveDef.fromPartial(side1, tightCP[index], tightCP[(index + 5) % 6], rSide1, 0, tightCurvePointLen);
    // Curve 1 L
    var cdl1 = CurveDef.fromPartial(side1, tightCP[index], tightCP[(index + 1) % 6], lSide, 0, tightCurvePointLen);
    // curve 2 R
    var cdr2 = CurveDef.fromPartial(side1, gentleCP[index], gentleCP[(index + 4) % 6], rSide2, 0, gentleCurvePointLen);
    // curve 2 L
    var cdl2 = CurveDef.fromPartial(side1, gentleCP[index], gentleCP[(index + 2) % 6], lSide2, 0, gentleCurvePointLen);
